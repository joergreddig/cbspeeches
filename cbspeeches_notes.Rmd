...
speeches_overview <- read_excel("data/Raw Data - All Speeches.xlsx", sheet = "Sheet1")
 
speeches_overview <- speeches_overview %>%
    select(c(Filename, City, Country, Event, Title)) %>% 
    rename(doc_id = Filename, city = City, country = Country, event = Event, title = Title)
